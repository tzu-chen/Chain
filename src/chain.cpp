...
MPO TranslationOp(const SiteSet& sites, bool inv) {
    ...
    if (not inv) {
        for (auto j : range1(N-1)) {
            // A cutoff like {"MaxDim", 100} is needed for this operator to be practically applicable.
...
//    identity_mpo.set(N, identity_mpo(N) * delta(long_link, prime(long_link)));
    identity_mpo.set(1, identity_mpo(1) * delta(prime(long_link), long_link));
    identity_mpo.set(N, identity_mpo(N) * delta(long_link, noPrime(long_link)));
    return  identity_mpo;
}
...
mydensityMatrixApplyMPOImpl(MPO const& K, MPS const& psi, Args args)
{
    ...
    {
        // Bottleneck
        // Size of E[j] goes like MPSBondDim^2 * MPOBondDim^2.
        // MPSBondDim is typically large when j is in the middle of the chain.
...
//
//
//// oldfixme: modify to allow for gates which are member functions of generic site type
