...
dmrg(MPS & psi, 
    ...
    {
    LocalMPO PH(H,args);
    Real energy = DMRGWorker(psi,PH,Hprojs,sweeps,args);
    return energy;
    }
...
dmrg(MPO const& H,
    ...
    auto energy = dmrg(psi,H,Hprojs,sweeps,args);
    return std::tuple<Real,MPS>(energy,psi);
    }
...
DMRGWorker(MPS & psi,
    ...
    {
    DMRGObserver obs(psi,args);
    Real energy = DMRGWorker(psi,PH,Hprojs,sweeps,obs,args);
    return energy;
    }
template<class LocalOpT>
Real
...
DMRGWorker(MPS & psi,
    ...
    MPO tproj = nmultMPO(prime(Hprojs[0]), Hprojs[1]);
    for(int i=2;i<length(psi);i++){
        tproj = nmultMPO(prime(tproj), Hprojs[i]);
    }
    for(int sw = 1; sw <= sweeps.nsweep(); ++sw)
        {
        ...
        if( args.defined("DistanceFile") ){
            FILE * dfile;
            dfile = fopen(args.getString("DistanceFile").c_str(), "a");
            //fprintf(eefile, "{\n");
            fprintf(dfile, "sweep %d: %.10f,\n",sw, errorMPOProd(psi, tproj, psi));
            //fprintf(eefile, "},\n");
            fclose(dfile);
        } else {
            //auto PH_init = PH;
            for (int i=1;i<=length(psi)-1;i++)
                {
                    psi = applyMPO(Hprojs[i], psi);
